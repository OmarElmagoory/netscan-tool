...
app = Flask(__name__)
CORS(app)
pdfmetrics.registerFont(TTFont('Arabic', '/home/kali/webscan/backend/arabic_font.ttf'))
...
@app.route("/")
def home():
    return jsonify({"status": "WebScan API running", "version": "2.0"})
...
def generate_report():
    ...
    sev_colors = {
        "critical": colors.HexColor('#ff3b5c'),
        "medium":   colors.HexColor('#ffb340'),
        "low":      colors.HexColor('#3b82f6'),
        "safe":     colors.HexColor('#00e676')
    }
    sev_bg = {
        "critical": colors.HexColor('#2a0a10'),
        "medium":   colors.HexColor('#2a1a00'),
        "low":      colors.HexColor('#0a1a2a'),
        "safe":     colors.HexColor('#0a2a1a')
    }
    buffer = io.BytesIO()
    ...
    story.append(Paragraph("Web Security Scanner v2.0", ss))
    story.append(Spacer(1, 0.4*cm))
    now = datetime.datetime.now().strftime("%Y-%m-%d %H:%M:%S")
    info = [
        [ar("الرابط المفحوص") if is_ar else "Target URL", url],
        [ar("تاريخ الفحص") if is_ar else "Scan Date", now],
        [ar("إجمالي النتائج") if is_ar else "Total Findings", str(summary.get("total",0))]
    ]
    it = Table(info, colWidths=[4*cm, 13*cm])
    it.setStyle(TableStyle([
        ('BACKGROUND',(0,0),(0,-1),colors.HexColor('#1a2235')),
    ...
    sl = [ar("حرجة") if is_ar else "Critical", ar("متوسطة") if is_ar else "Medium",
          ar("منخفضة") if is_ar else "Low", ar("آمن") if is_ar else "Safe"]
    sv = [str(summary.get("critical",0)), str(summary.get("medium",0)),
          str(summary.get("low",0)), str(summary.get("safe",0))]
    ...
    smt.setStyle(TableStyle([
        ...
        ('PADDING',(0,0),(-1,-1),12),
        ('BACKGROUND',(0,0),(0,0),colors.HexColor('#ff3b5c')),
        ('BACKGROUND',(1,0),(1,0),colors.HexColor('#ffb340')),
        ('BACKGROUND',(2,0),(2,0),colors.HexColor('#3b82f6')),
        ('BACKGROUND',(3,0),(3,0),colors.HexColor('#00e676')),
        ('TEXTCOLOR',(0,0),(-1,0),colors.white),
        ('BACKGROUND',(0,1),(0,1),colors.HexColor('#2a0a10')),
        ('BACKGROUND',(1,1),(1,1),colors.HexColor('#2a1a00')),
        ('BACKGROUND',(2,1),(2,1),colors.HexColor('#0a1a2a')),
        ('BACKGROUND',(3,1),(3,1),colors.HexColor('#0a2a1a')),
        ('TEXTCOLOR',(0,1),(0,1),colors.HexColor('#ff3b5c')),
        ('TEXTCOLOR',(1,1),(1,1),colors.HexColor('#ffb340')),
        ('TEXTCOLOR',(2,1),(2,1),colors.HexColor('#3b82f6')),
        ('TEXTCOLOR',(3,1),(3,1),colors.HexColor('#00e676')),
        ('GRID',(0,0),(-1,-1),1,colors.HexColor('#0a0e1a')),
    ]))
    story.append(smt)
    ...
    for r in results:
        ...
        hr = [[
            ...
            Paragraph(name, ParagraphStyle('nh', fontSize=10, fontName=fb, textColor=colors.white))
        ]]
        ht = Table(hr, colWidths=[2.2*cm, 14.8*cm])
        ht.setStyle(TableStyle([
            ('BACKGROUND',(0,0),(0,0),sc),
            ('BACKGROUND',(1,0),(1,0),colors.HexColor('#1a2235')),
            ('PADDING',(0,0),(-1,-1),8),
            ('VALIGN',(0,0),(-1,-1),'MIDDLE'),
        ]))
        story.append(ht)
        ...
        if fix:
            ...
            ft.setStyle(TableStyle([
                ...
                ('PADDING',(0,0),(-1,-1),8),
                ('LINEBELOW',(0,0),(-1,-1),0.5,colors.HexColor('#1e3a5f'))
            ]))
            story.append(ft)
        story.append(Spacer(1, 0.15*cm))
    story.append(Spacer(1, 0.5*cm))
    story.append(Paragraph(
        ar("تم إنشاء هذا التقرير بواسطة Web Security Scanner v2.0") if is_ar else "Generated by Web Security Scanner v2.0",
        ParagraphStyle('footer', fontSize=8, fontName=fn, textColor=colors.HexColor('#64748b'), alignment=1)
    ))
    doc.build(story)
    buffer.seek(0)
